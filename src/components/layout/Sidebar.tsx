...
export function BottomNav({ userRole }: BottomNavProps) {
  ...
  const mobileItems = isAdmin ? adminNavItems : navItems.slice(0, 5);

  return (
    <nav className={`fixed bottom-0 left-0 right-0 z-40 flex md:hidden border-t ${
      isAdmin ? 'bg-[#271E11]/95 border-amber-700/60 text-amber-100 backdrop-blur-md shadow-lg' : 'bg-white border-slate-200'
    }`}>
      {mobileItems.map((item) => {
        const isActive = pathname === item.href || pathname.startsWith(item.href + '/');
        const activeTextColor = isAdmin ? 'text-amber-400 font-bold' : 'text-[#4F46E5] font-semibold';
        const inactiveTextColor = isAdmin ? 'text-amber-200/70 hover:text-amber-100' : 'text-slate-500 hover:text-slate-700';
        const activeBarColor = isAdmin ? 'bg-amber-400' : 'bg-[#4F46E5]';
        ...
        return (
          <Link
            ...
            href={item.href}
            className={[
              'flex-1 flex flex-col items-center justify-center py-2 gap-1 text-[11px]',
          ...
          >
            ...
            {isActive && (
              <span className={`absolute top-0 w-8 h-1 ${activeBarColor} rounded-b-full`} />
            )}
          </Link>
        );
      })}
